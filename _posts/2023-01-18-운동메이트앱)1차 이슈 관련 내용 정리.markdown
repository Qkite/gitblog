...
1. 구현 내용
   1. BEncrypterEncoder로 비밀번호 암호화 진행
   2. 결과를 모두 Response이 형태로 반환: AppException도 던지면 ExceptionManager에서 Response의 형태로 감싸서 반환
   3. 아이디 중복, 닉네임 중복 모두 Exception으로 던졌지만 닉네임은 추후에 중복 확인 버튼 생성으로 수정할 계획
   4. 관심 종목을 설정할 수 있도록 함 -> of(Entity로 변환하는 메소드) 사용해서 리팩토링 진행해야 함
   5. enum type으로 설정되어 있어 중복 선택이 어려움 -> 종목을 Entity의 형태로 변환하는 것이 필요
   6. 프로필 기능과 함께 회원 정보 수정도 필요할 것 같음
